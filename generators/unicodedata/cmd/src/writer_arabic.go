...
// TODO: probably use packtab as well for joinings
...
func packPUAMap(mapping [][2]rune) []int {
	var maxRune rune
...
func generateArabicPUAMapping(simp, trad [][2]rune, w io.Writer) {
	...
	codeSimp := packtab.PackTable(simpM, 0, 9).Code("puaSimp")
	codeTrad := packtab.PackTable(tradM, 0, 9).Code("puaTrad")

	fmt.Fprintln(w, codeSimp)
	fmt.Fprintln(w, codeTrad)
}
